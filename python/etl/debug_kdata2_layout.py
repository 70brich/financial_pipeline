...
def main() -> None:
    candidates = (
        sorted((INPUT_DIR / "KDATA2").glob("*.xls"))
        + sorted((INPUT_DIR / "KDATA2").glob("*.xlsx"))
        + sorted((INPUT_DIR / "KDATA2").glob("*.xlsm"))
    )
    if not candidates:
        raise FileNotFoundError("No KDATA2 workbook found under data/input/KDATA2")

    file_path = candidates[0]
    workbook = xlrd.open_workbook(file_path.as_posix(), encoding_override="cp949")

    print(f"FILE={file_path}")
    print(f"SHEET_COUNT={workbook.nsheets}")
    ...
    for sheet_index in range(workbook.nsheets):
        ...
        for row_index in range(min(15, sheet.nrows)):
            rendered = [
                ...
                for col_index in range(min(15, sheet.ncols))
            ]
