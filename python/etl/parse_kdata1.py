...
KDATA1_INPUT_DIR = INPUT_DIR / "KDATA1"


@dataclass(frozen=True)
class Kdata1FileContext:
    source_file_id: int
    source_group: str
    relative_path: str
    file_name: str
...
def choose_excel_engine(file_path: Path) -> str:
    ...
    if suffix in {".xlsx", ".xlsm"}:
        return "openpyxl"
    raise ValueError(f"Unsupported KDATA1 workbook extension: {suffix}")
...
def derive_quarter_fields(date_value: object) -> tuple[str, int, int, str]:
    if isinstance(date_value, (int, float)):
        raise ValueError(f"Numeric values are not accepted as KDATA1 date headers: {date_value}")

    parsed = pd.to_datetime(date_value)
    if pd.isna(parsed):
        raise ValueError(f"Could not parse KDATA1 date value: {date_value}")

    fiscal_year = int(parsed.year)
    fiscal_quarter = ((int(parsed.month) - 1) // 3) + 1
...
def is_metric_like(value: object) -> bool:
    if value is None:
        return False
    if isinstance(value, str):
        stripped = value.strip()
...
def repair_mojibake_text(value: str) -> str:
    ...
    candidates = [stripped]
    for source_encoding, target_encoding in (("cp949", "utf-8"), ("latin1", "utf-8")):
        try:
            repaired = stripped.encode(source_encoding).decode(target_encoding)
    ...
    def score(text: str) -> tuple[int, int]:
        ...
        return (korean_count, -replacement_penalty)
...
def get_metric_columns(header_row: list[object]) -> list[tuple[int, str]]:
    ...
    for column_index, header_value in enumerate(header_row[1:], start=1):
        metric_name = "" if header_value is None else repair_mojibake_text(str(header_value))
        if not metric_name:
            continue
...
def melt_kdata1_sheet(
    ...
    header_row_index = 0
    header_row = dataframe.iloc[header_row_index].tolist()
    metric_columns = get_metric_columns(header_row)
    if not metric_columns:
        raise ValueError(f"No metric columns found in KDATA1 sheet: {sheet_name}")
    ...
    for row_index in range(1, len(dataframe)):
        ...
        date_raw, fiscal_year, fiscal_quarter, period_label_std = derive_quarter_fields(date_value)
            ...
            records.append(
                {
                    ...
                    "normalized_quarter_label": period_label_std,
...
def load_kdata1_workbook(file_path: Path) -> dict[str, pd.DataFrame]:
...
def fetch_kdata1_source_files(engine) -> list[Kdata1FileContext]:
...
def parse_kdata1_source_files(engine, input_dir: Path = INPUT_DIR) -> int:
    source_files = fetch_kdata1_source_files(engine)
    inserted_rows = 0

    with engine.begin() as connection:
        for context in source_files:
            workbook_path = input_dir / context.relative_path
            sheet_frames = load_kdata1_workbook(workbook_path)
            records: list[dict] = []
            for sheet_name, dataframe in sheet_frames.items():
                records.extend(melt_kdata1_sheet(dataframe, context=context, sheet_name=sheet_name))
