...
def main() -> None:
    ...
    with engine.begin() as connection:
        kdata2_files = connection.execute(
            text(
                """
        ...
        total_kdata2_rows = connection.execute(
            text(
                """
                SELECT COUNT(*)
        ...
        anomaly_summary = connection.execute(
            text(
                ...
                SELECT
                    ...
                    SUM(CASE WHEN is_estimate = 1 THEN 1 ELSE 0 END) AS estimate_rows,
                    SUM(CASE WHEN is_estimate IS NULL THEN 1 ELSE 0 END) AS null_is_estimate_rows
                FROM raw_observation
                WHERE source_group = 'KDATA2'
                """
            )
        ...
        row_counts_by_file = connection.execute(
            text(
                ...
                LEFT JOIN raw_observation ro
                 ...
                 AND ro.source_group = 'KDATA2'
                WHERE sf.source_group = 'KDATA2'
    ...
    print("KDATA2 load inspection")
    print(f"Database: {DEFAULT_SQLITE_DB_PATH}")

    print_section("KDATA2 source_file rows")
    if kdata2_files:
        for row in kdata2_files:
            print(f"[{row.source_file_id}] {row.file_name} | {row.relative_path}")
    else:
        print("No KDATA2 source_file rows found.")

    print_section("Raw row count by KDATA2 source_file")
    ...
    print_section("Total KDATA2 raw_observation rows")
    print(f"{int(total_kdata2_rows)}")

    print_section("Recent KDATA2 raw rows")
    if recent_rows:
        for row in recent_rows:
            print(
                ...
                f"is_estimate={row.is_estimate}"
            )
    else:
        print("No KDATA2 raw_observation rows found.")

    print_section("KDATA2 anomaly summary")
    ...
    print(f"is_estimate=1 rows: {int(anomaly_summary.estimate_rows or 0)}")
    print(f"is_estimate null rows: {int(anomaly_summary.null_is_estimate_rows or 0)}")
